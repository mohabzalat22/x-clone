<script setup>
    ...
    const props = defineProps({
        ...
        image: String,
        tank: Number,
        type: String, // manual vs automatic
        capacity: Number, // people can ride
        dailyPrice: Number,
        to: String
    });
...
<template>
    <div class="p-5 bg-white w-72 h-96 flex flex-col rounded-lg select-none">
            <div class="flex justify-between">
                <strong class="text-black capitalize">{{ name }}</strong>
                <div @click="loved =! loved">
                    <svg class="w-6 h-6" :class = "{'hidden': loved}" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                         <path d="M12.62 20.81C12.28 20.93 11.72 20.93 11.38 20.81C8.48 19.82 2 15.69 2 8.68998C2 5.59998 4.49 3.09998 7.56 3.09998C9.38 3.09998 10.99 3.97998 12 5.33998C13.01 3.97998 14.63 3.09998 16.44 3.09998C19.51 3.09998 22 5.59998 22 8.68998C22 15.69 15.52 19.82 12.62 20.81Z" stroke="#90A3BF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    <!-- active heart -->
                    <svg class="w-6 h-6" :class="{'block': loved , 'hidden': !loved}" viewBox="0 0 24 24" fill="red" xmlns="http://www.w3.org/2000/svg">
            ...
            <small class="text-gray-400 capitalize">{{ model }}</small>
        <div class="flex-grow flex justify-center items-center my-2">
            <div class="relative z-10">
                <img :src='`${image}`' alt="" class="z-20 mx-auto">
                <div class="h-4 w-11/12 -bottom-5 start-0 rounded-full absolute z-30 bg-gradient-to-t from-gray-200 to-white"></div>
            </div>
        </div>
        <div class=" flex justify-between">
            <div class="p-1 flex items-end">
                ...
                <span class="text-sm text-gray-400 ms-2">{{ tank }}L</span>
            </div>
            <div class="p-1 flex items-end">
                <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span class="text-sm text-gray-400 ms-2 capitalize">{{ type }}</span>
            </div>
            <div class="p-1 flex items-end">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M19.9901 7.34001C20.1501 9.28001 18.7701 10.98 16.8601 11.21C16.8501 11.21 16.8501 11.21 16.8401 11.21H16.8101C16.7501 11.21 16.6901 11.21 16.6401 11.23C15.6701 11.28 14.7801 10.97 14.1101 10.4C15.1401 9.48001 15.7301 8.10001 15.6101 6.60001C15.5401 5.79001 15.2601 5.05001 14.8401 4.42001C15.2201 4.23001 15.6601 4.11001 16.1101 4.07001C18.0701 3.90001 19.8201 5.36001 19.9901 7.34001Z" fill="#90A3BF"/>
                ...
                <span class="text-sm text-gray-400 ms-2 text-nowrap">{{ capacity }} People</span>
            </div>
        </div>
        <div class="flex items-center justify-between mt-2">
            <div class="flex py-4 items-end">
                <h2 class="text-xl font-semibold">${{dailyPrice?.toFixed(2)}}/</h2>
                <p class="ms-1 text-gray-400">day</p>
            </div>
            <div>
                <RouterLink class= "bg-blue-600 text-white text-bold px-2 py-2 rounded-lg" :to="`${to}`">Rent Now</RouterLink>
            </div>
        </div>
    </div>
</template>
